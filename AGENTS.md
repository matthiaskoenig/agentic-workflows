...
- `main` is protected: no direct push, every change goes through a pull request whose `build`, `tests`, `ruff` and `ty` checks pass. Work on a branch, open a pull request with `gh pr create`.
