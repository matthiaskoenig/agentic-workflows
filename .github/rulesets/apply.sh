#!/usr/bin/env bash
#
# Apply the repository policies: the merge settings of the repository and the
# rulesets in this directory. The script is idempotent, i.e., a ruleset which
# already exists is updated instead of added a second time, so it can be run
# again after every change of the json files.
#
...
REPO="${1:-matthiaskoenig/agentic-workflows}"
