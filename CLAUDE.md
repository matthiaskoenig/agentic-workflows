...
[uv](https://docs.astral.sh/uv/) manages the environment; the Python version is in `.python-version`.

## Layout

- `docs/` - all site content, one markdown file per page
- `docs/concepts/` - harnesses, instruction files, skills, orchestration
- `docs/best-practices/` - the development loop, context management, standards
- `zensical.toml` - site config and navigation (`nav`); add new pages here
- `tests/test_docs.py` - strict build, nav and link checks, writing rules; runs in CI as the `tests` check
- `release-notes/` - one note per tagged release
- `.github/rulesets/` - branch protection as JSON, applied with `apply.sh`
...
- Every new page must be added to `nav` in `zensical.toml`, otherwise it is unreachable.
...
- `main` is protected: no direct push, every change goes through a pull request whose `build` check passes. Work on a branch, open a pull request with `gh pr create`.
